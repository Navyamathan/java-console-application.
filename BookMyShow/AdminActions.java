...
class AdminActions
{
    ...
    public Admin adminLogin(Scanner s)
    {
        ...
        for (Admin tempAccount : BookMyShow.getAdminArrayList())
        {
            while (attempts < 3)
            {
                System.out.println();
    ...
    public void addTheatre(Scanner s)
    {
        ...
        int screenCount = s.nextInt();
        s.nextLine();
        Theatre t = new Theatre(theatreName, theatreLocation, screenCount);
        BookMyShow.getTheatreArrayList().add(t);
        for (int i = 1; i <= screenCount; i++)
        {
            ...
            while(true)
            {
                ...
                HashMap<Character,ArrayList<String>> seats = u.calculateGrid(seatsCount,seatsGrid);
                if(seats!=null)
                {
                    t.getScreenArrayList().add(new Screen(screenName,seatsCount,seats));
                    break;
                }
            }
        }
    ...
    public void viewTheatreDetails()
    {
        ...
        System.out.println();
        for(Theatre tempTheatre : BookMyShow.getTheatreArrayList())
        {
            ...
            int i=1;
            for(Screen tempScreen : tempTheatre.getScreenArrayList())
            {
                System.out.println();
                ...
                u.printSeats(tempScreen.getSeatsGrid());
                i++;
            }
        }
    }
    ...
    public void addMovie(Scanner s)
    {
        ...
        localDate = LocalDate.parse(date,Theatre.getLocalDateFormatter());
        System.out.print("Enter The Duration( HH:mm ): ");
        String duration = s.nextLine();
        LocalTime localTime = LocalTime.parse(duration,Theatre.getLocalTimeFormatter());
        ...
        while(true)
        {
            ...
            for(Theatre tempTheatre : BookMyShow.getTheatreArrayList())
            {
                if (tempTheatre.getTheatreName().equals(theatreName))
                {
                    theatre = tempTheatre;
                    for (Screen tempScreen : tempTheatre.getScreenArrayList())
                    {
        ...
        while(true)
        {
            ...
            for(Screen tempScreen : theatre.getScreenArrayList())
            {
                if (screenName.equals(tempScreen.getScreenName()))
                {
                    screen = tempScreen;
                }
            }
        ...
        while(true)
        {
            ...
            LocalTime localStartingTime = LocalTime.parse(startingTime,Theatre.getLocalTimeFormatter());
            LocalTime localEndingTime = localStartingTime.plusHours(localTime.getHour()).plusMinutes(localTime.getMinute()).plusMinutes(30);
            for(Movie tempMovie : Theatre.getMovieArrayList())
            {
            ...
            else
            {
                ...
                Theatre.getShowArrayList().add(show);
                System.out.println("Movie Added Successfully!");
                break;
            }
        }
        Theatre.getMovieArrayList().add(new Movie(movieName,localDate,localTime,price,location,theatre,screen,show));
    }
