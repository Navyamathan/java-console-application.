...
class CustomerActions
{
    ...
    public void customerAction(Scanner s, Customer customer)throws CloneNotSupportedException // this exception comes cause of we use Clone built-in function in the project
    {
        System.out.println();
    ...
    public static Customer customerLogin(Scanner s) {
        ...
        for (Customer tempAccount : Atm.getCustomerArrayList()) // for loop for checking the account number and pin one by one in the customer arraylist
        {
            while (attempts < 3) // this while maintain for managing the attempt of the login process
            {
                ...
                if (accNo.equals(tempAccount.getAccountNumber())) //checks that the customer given account number is equal to the data in the customer arrayList
                {
    ...
    {
        System.out.println("Your Current Account Balance Is: ₹ " +String.format("%,.2f",customer.getAccountBalance())); // using String format function to format the double variable
        System.out.println();
    }
    ...
    public void depositCash(Scanner s, Customer customer) {
        ...
        if (calculate == amount) // checks is the given amount and denomination for amount is equal
        {
            ...
            customer.setAccountBalance(customer.getAccountBalance()+amount); // update the money in the customer balance by calling setAccountBalance method
            ...
            Customer.getTransactionArrayList().add(new Transaction(customer.getAccountNumber(), "Deposit", amount)); // add the deposited information to the transaction arraylist
            System.out.println();
        }
    ...
    public void withdrawCash(Scanner s,Customer customer)throws CloneNotSupportedException // this exception comes cause of we use Clone built-in function in the project
    {
        ...
        if(amount < customer.getAccountBalance()) // checks if the withdrawal amount is lesser than the customer account balance
        {
        ...
        else // if condition is false then else block will be executed
        {
            ...
            if(remainder == 0) // finally check if the remainder is 0
            {
                ...
                customer.setAccountBalance(customer.getAccountBalance() - withdrawAmount); // minus the withdrawal amount in the customer account balance and set that amount by calling setAccountBalance function
                ...
                Customer.getTransactionArrayList().add(new Transaction(customer.getAccountNumber(), "Withdraw", withdrawAmount)); // add the withdrawal information to the transaction arraylist
    ...
    public void changePIN(Scanner s,Customer customer)
    {
        ...
        {
            customer.setAccountPin(newPassword); //set the new password as a pin by call the method setAccountPin
            System.out.println();
    ...
    public void transactions(Customer customer)
    {
        ...
        else // if the Transaction arraylist is not empty then else block will be executed
        {
            ...
            {
                if (customer.getAccountNumber().equals(transaction.getAccountNumber())) //checks the given customer account number equals to the data in the transaction array list
                {
