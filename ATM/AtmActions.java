...
class AtmActions
{
    ...
    public void start()throws  CloneNotSupportedException // this exception comes cause of we use Clone built-in function in the project
    {
        ...
        while (true) // while loop for run iteratively to show the choice again and again until the user give exit option
        {
            ...
            if (n == 1)
            {
                Atm.getAdminArrayList().add(new Admin("Navya","1995")); // add the Admin data to the Admin arraylist
                Admin currentAdmin = AdminActions.adminLogin(s); // call the adminLogin function By the Class name because this function declared as a static and store the value to the variable that the function returns

                if (currentAdmin != null ) //check the variable is not null
                {
                    if (currentAdmin.getAdminPassword() == null) // check the password is null in the variable of the type Admin object
                    {
                        System.out.println("Access denied. You have used all login attempts.");
                    }
                    else // if "if" statement is false then else will be executed
                    {
                        System.out.println();
            ...
            {
                Customer currentCustomer = CustomerActions.customerLogin(s);// call the coustomerLogin function By the Class name because this function declared as a static and store the value to the variable that the function returns
                if (currentCustomer != null )//check the variable is not null
                {
                    if (currentCustomer.getAccountPin() == null) // check the password is null in the variable of the type Admin object
                    {
                        System.out.println("Access denied. You have used all login attempts.");
                    }
                    else // if "if" statement is false then else will be executed
                    {
                        System.out.println();
                        System.out.println("Login successful!");
                        c.customerAction(s, currentCustomer); // call the coustomerAction function by using the CustomerActions object c
                    }
                }
                else // if "if" statement is false then else will be executed
                {
